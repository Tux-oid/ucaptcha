<?php
class Graph implements PluginInterface
{
	...
	function generateImage($canvas)
	{
		...
		for ($i=$liney;$i>0;$i--)
		{
			...
			imagefttext($canvas, 8, rand (-45,45), 3, $i*60/$liney, $color, $this->ucaptcha->getCaptchaFontPath()."/LiberationMono-Bold.ttf", $liney-$i);
		}
		for ($i=0;$i<$linex;$i++)
		{
			$color = imagecolorallocate($canvas, rand(100,255),rand(100,255) , rand(100,255));
			...
			$color = imagecolorallocate($canvas, rand(100,255),rand(100,255) , rand(100,255));
			imagefttext($canvas, 8, rand (-45,45), 30 + $i*160/$linex, 80, $color, $this->ucaptcha->getCaptchaFontPath()."/LiberationMono-Bold.ttf", $i);
		}
//		$color = imagecolorallocate($canvas, rand(100,255),rand(100,255) , rand(100,255));
		imagesetthickness($canvas, 1);
		$x=rand(0,$linex-1);
		...
		if ($var)
		{
			imagefttext($canvas, 20, rand (-45,45), 40+20, 50, $color, $this->ucaptcha->getCaptchaFontPath()."/LiberationMono-Bold.ttf", "y=?");   
		}
		else
		{
			imagefttext($canvas, 12, rand (-45,45), 100, 70, $color, $this->ucaptcha->getCaptchaFontPath()."/LiberationMono-Bold.ttf", "X=?");   
		}
